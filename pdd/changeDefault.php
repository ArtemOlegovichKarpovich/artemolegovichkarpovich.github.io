...
$month = (int) date('m');
$year_start = 2019;
$year_for_request = 2018;

shell_exec("cd /D D:\php_localhost\pdd");
shell_exec("weeks_finder.exe {$year_for_request}");
$weeks_file = fopen("weeks.txt", 'r');
$weeks = [];
$dates = [];
$index_of_current_week = 0;
//if ($year_for_request == $year - 1 || $year_for_request == $year){
if (isset($_GET["hiddenweek"])){
    $_POST['week'] = $_GET["hiddenweek"] . "\r\n";
    $_POST["studyyearMAIN"] = $_GET["studyyear"];
}
if ((!isset($_POST['week']))){
    $year_for_request = (int)substr($_POST["studyyearMAIN"], 0, 4);
    shell_exec("weeks_finder.exe {$year_for_request}");
    $weeks_file = fopen("weeks.txt", 'r');
    if (($month < 9 && $year_for_request == $year - 1) || ($month >= 9 && $year_for_request == $year)){
    ...
    if (isset($_POST['studyyear'])){
        $year_for_request = (int)$_POST["studyyear"];
    }
    else{
        $request = explode(".", substr($_POST["week"], 3, 7));
        $month_for_request = (int)$request[0];
        if($month_for_request < 9){
            $year_for_request = (int)$request[1] - 1;
        }
        else{
            $year_for_request = (int)$request[1];
        }
        //$year_for_request = (int)$_POST["weeks"];
    }
    shell_exec("weeks_finder.exe {$year_for_request}");
    $weeks_file = fopen("weeks.txt", 'r');
    while(!feof($weeks_file)){
        ...
        $str = str_replace('t', '', $str);
        $weeks[] = $str;
        if (($str."\n" == $_POST['week']) || ($str == $_POST['week'])){
            $index_of_current_week = count($weeks) - 1;
        }
    }
}
fclose($weeks_file);
...
echo "</head>";
echo "<body>";
echo "<form class=\"filter-from-ver-1\" id=\"headForm\" style=\"margin-bottom: 1px\" method=\"post\" action=\"changeDefault.php\">";
echo "        Учебный год";
echo "        <select id=\"studyyearMAIN\" style=\"background-color:#E0E1B2;\" name=\"studyyearMAIN\" onchange=\"this.form.submit();\">";

if(!isset($_POST["studyyearMAIN"])){
    $changed_year = (int) substr($weeks[0], 6);
}
else{
    $changed_year = substr($_POST['studyyearMAIN'], 0, 4);
}
$buf = $year_start;
while($buf <= $year + 1){
    $prev = $buf - 1;
    $second = $buf;
	if ($prev == $changed_year){
...
$time_of_pairs = ['09.00', '09.50', '10.40', '11.30', '13.00', '13.50', '14.40', '15.30'];
$days = [1 => 'Понедельник',
         2 => 'Вторник',
         3 => 'Среда',
         4 => 'Четверг',
        ...
        if ($groups[0]){
            for($count = 1, $group=-1; $count < (count($groups))*2 + 1; ++$count){
                ...
                if ($timetable[$i] != "%%%%%"){
                    ...
                    $order_max = (explode("%", $disciplines_hours[str_replace("\r\n", "", $groups_courses[$group]) . "&" . $show_str[4]] . "%0")[$types_match2[$show_str[3]]]);
